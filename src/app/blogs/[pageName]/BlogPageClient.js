...
export default function BlogPageClient({ blogData, cardData }) {
  // Generate random colors for charts when component mounts
  const [chartColorAssignments] = useState(() => 
    Array(blogData?.charts?.length || 0).fill(null).map(() => getRandomColor())
  );
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
                {blogData.charts.map((chart, index) => (
                  <AnimatedCircleChart
                    ...
                    percentage={chart.percentage || 0}
                    title={chart.title || ""}
                    color={chartColorAssignments[index]}
                  />
                ))}
              </div>
            </div>
